...
func messageCreateHandler(s *discordgo.Session, m *discordgo.MessageCreate, channelID string, config *utils.Config) {
    ...
    if m.ChannelID == channelID {
        // Create a JSON object to hold the content and attachment URLs
        ...
        if len(m.Attachments) > 0 {
            ...
            for _, attachment := range m.Attachments {
                ...
                content.(map[string]interface{})["attachments"] = append(content.(map[string]interface{})["attachments"].([]map[string]string), map[string]string{"url": decodedURL})
            }
        } else {
            content = m.Content
        }
        ...
        event := nostr.NostrEvent{
            ...
            Kind:      1,
            Content:   contentStr, // Correctly serialized content
